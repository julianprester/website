...
export default function (eleventyConfig) {
  ...
  eleventyConfig.addGlobalData("eleventyComputed", {
    description(data) {
      ...
      if (url === "/writing/")
        return "Latest writing and ideas by Julian Prester on academia, technology, and digital work."
      if (url === "/bookshelf/")
        return "Books that inform Julian Prester's research on digital work with personal reading notes and reviews."
      if (url.startsWith("/bookshelf/") && url !== "/bookshelf/")
        return `${title}${author ? ` by ${author}` : ""} — book notes and review by Julian Prester`
      if (url.startsWith("/writing/") && url !== "/writing/" && !url.startsWith("/tags/"))
        return `${title} — a post by Julian Prester on academia, technology, and digital work`
      if (url.startsWith("/links/") && url !== "/links/" && !url.startsWith("/tags/"))
        return `${title} — a link shared by Julian Prester`
      if (url.startsWith("/news/")) return `${title} — news from Julian Prester`
      if (url.startsWith("/tags/"))
        return `Content tagged "${url.replace("/tags/", "").replace(/\/$/, "")}" on Julian Prester's website`
      return fallback
    },
  })
